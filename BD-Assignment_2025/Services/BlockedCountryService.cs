...
public sealed record BlockedCountryService : IBlockedCountryService
{
        ...
        var result = await _blockedCountryRepository.PaginateAsync(
            ...
            bc =>
                bc.Value.Code.ToLower()
                    .Contains(string.IsNullOrWhiteSpace(search) ? "" : search.ToLower()),
            cancellationToken
        );
    ...
    )
    {
        ...
        if (
            await _blockedCountryRepository.IsAnyExist(c =>
                c.Value.Code.ToLower().Equals(countryCode.ToLower()) && c.Value.Duration is not null
            )
        )
    ...
    {
        int Page = page is > 0 ? page.Value : 1;
        int PageSize = pageSize is > 0 ? pageSize.Value : 10;
